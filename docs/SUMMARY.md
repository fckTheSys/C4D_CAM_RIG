...
При изменении логики обновляйте **`PLUGIN_VERSION`** в `config.py`, **`EMBEDDED_RUNTIME_VERSION`** в `tag_embedded.py`, запись в **`active/VERSIONS.txt`**, при необходимости **`active/CHANGELOG.md`**.
